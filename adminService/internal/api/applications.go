...
func (_ *Handler) GetApplications(ctx *context.Context) {
	...
	if search == "my_tasks" {
		...
		uid = int(resp.Uid)
		url = fmt.Sprintf("http://localhost:8082/get/applications?search=%s&id=%d", search, uid)
	}

	if uid == 0 {
		url = fmt.Sprintf("http://localhost:8082/get/applications")
	}
...
func (_ *Handler) CancelApplication(ctx *context.Context) {
	...
	if err != nil {
		log.Println("Ошибка выполнения POST2 запроса: ", err)
		ctx.Response.WriteHeader(400)
		return
	}

	if err = json.NewDecoder(req.Body).Decode(&resp); err != nil {
		log.Println("Ошибка декодирования json2: ", err)
		ctx.Response.WriteHeader(400)
		return
	}
	fmt.Println(resp.Ok)
